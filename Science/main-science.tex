...
\begin{sciabstract}
Continuous response ­threshold functions to coordinate collaborative tasks in multi-­agent systems are commonly employed models in a number of fields including ethology, economics, and robotics. While ample empirical evidence exists for the response­ threshold model in predicting and matching observed swarm behavior for task ­assignment in social insects, there has been no formal argument as to why natural swarms use this approach, and why response thresholds should be used for engineering artificial swarms. In this paper we show by formulating task allocation as a Global Game that continuous response ­threshold functions used for communication­ free task ­assignment result in system ­level Bayesian Nash Equilibria.  Building up on these results, we show that individual agents not only do not need to communicate with each other, but also do not need to model each other's behavior, which makes this coordination mechanism accessible to very simple agents, suggesting their prevalence in nature, not limited to social insects, and motivating their further use in an engineering context. 
\end{sciabstract}
...
\end{enumerate}
This algorithm is extremely simple, and can be implemented on systems with a wide range of capabilities, yet leads to a BNE, which we show in Theorem 1 in the supplemental materials. 
...
\end{equation}
where $\Phi$ is the cumulative distribution function (cdf) of a standard Gaussian, which is illustrated numerically in Figure 2, and proved in Theorem 2 in the supplementary materials.  Therefore, despite agents using deterministic threshold strategies, their \emph{aggregate behavior} would appear to an outside observer as a continuous sigmoid threshold function instead of numerous discrete thresholds.
...
To show that there exists a sensible threshold strategy for the class of tasks with concurrent benefit in Theorem 1, we will first show that the best response to threshold strategies is a threshold strategy (Lemma 1), and then show that there exists an equilibrium of threshold strategies\cite{Carlsson1993, Morris2000} (Lemma 2).
