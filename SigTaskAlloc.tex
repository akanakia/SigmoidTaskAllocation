...
\centering\includegraphics[width=\textwidth]{figures/setup.png}
\caption{A visual representation of the collaboration experiment being studied.\label{fig:setup}}
\end{subfigure}~
\begin{subfigure}{.5\textwidth}
\centering\includegraphics[width=.9\textwidth]{figures/droplettop.jpg}
\centering\caption{A single Droplet robot with its outer shell removed. Indicated are an RGB color sensor, 6 IR sensors/LEDs and 3 motors.\cite{farrow14, klingner14}}\label{fig:droplettop}
\end{subfigure}
\end{figure*}
...
\begin{figure*}[!htb]
\centering\begin{subfigure}{.5\textwidth}
\centering\includegraphics[width=\textwidth]{figures/sigmoid2.png}
\caption{Changing $\tau$ offsets the curve along the $x$ axis, allowing to set the desired mean team size.}\label{}
\end{subfigure}~
\centering\begin{subfigure}{.5\textwidth}
\centering\includegraphics[width=\textwidth]{figures/sigmoid1.png}
...
We use the Droplet swarm robot platform to perform real experiments to study the effects of using the proposed task allocation scheme on a physical system. The Droplets are small individually simple robots capable of omni-directional motion and communication (via IR) as well as RGB sensing, as seen in Figure \ref{fig:droplettop}.

I our experiment we assume that all agents are currently at a single collaboration site and measure the corresponding collaboration rates for a team of 6 robots while varying values of $\tau$ and $\theta$. Each agent is individually running the algorithm described in Alg.\ref{alg:sigalg}. A collaboration event is recognized by having the all the robots turn on their green LEDs for 3 seconds. After such a collaboration event, each agent resets its group size estimate and runs Alg.\ref{alg:sigalg} again. 

We ran 15 experiments for all combinations of $\tau = 2,3,4,5$ and $6$, and $\theta = 0.1, 1$ and $10$. Each experiment lasted 30 minutes and an overhead camera system was set up to detect collaboration events (see Figure \ref{fig:camera}). The collaboration rate was a value computed by counting the number of collaborations over the course of each 30 minute experiment and normalizing to collaborations per minute. The results of these experiments are seen in Figure \ref{fig:expdat}.

Although we observe qualitatively similar results to the micro-macro predictions, there were a number of factors that affected the number of collaborations happening in the real robot experiments. 

\begin{enumerate}
	\item We assume perfect group size estimation in the micro-level simulations which is not the case here. Due to imperfect communication many agents often incorrectly estimated the size of the group (which was always 6) that they were in and hence computed different collaboration probabilities using the sigmoid function.
	\item The overhead camera system set up for automatically counting collaborations between agents used the \emph{Roborealm} vision software that, in some cases, failed to detect and/or intermittently detected a robot's green led turning on. This may have resulted in lower than actual collaborations being counted during the course of the 30 minute experiment.  
	\item To account for the vision software's detection errors, the raw data gathered from each experiment was de-bounced and passed through a low-pass filter to expose real collaboration events while eliminating observation error. 
\end{enumerate}

\begin{figure}[!htb]
\begin{subfigure}{.5\textwidth}
\centering\includegraphics[width=\textwidth]{figures/realsimexp.png}
\centering\caption{The solid lines show collaborations per min, over 30min, for a group of 6 real robots as the desired group size is varied from 2 to 6 and the slope of $\sig$ is varied between $0.1, 1$ and $10$. The dashed lines indicate simulation results with the same parameters.}\label{fig:expdat}
\end{subfigure}~
\begin{subfigure}{.5\textwidth}
\centering\includegraphics[width=.8\textwidth]{figures/droplets.png}
\caption{The Droplets swarm robotic platform \cite{farrow14,klingner14}.\label{fig:dropletexp}}
\end{subfigure}
\end{figure}
