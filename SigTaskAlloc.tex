...
We ran 15 experiments for all combinations of $\tau = 2,3,4,5$ and $6$, $\theta = 0.1, 1$ and $10$. Each experiment lasted 30 minutes and an overhead camera system was set up to detect collaboration events (see Figure \ref{fig:camera}). The collaboration rate was a value computed by counting the number of collaborations over the course of each 30 minute experiment and normalizing to collaborations per minute. The results of these experiments are seen in Table \ref{tab:realexpres}.

Although observe qualitatively similar results to the micro-macro predictions, there were a number of factors that affected the number of collaborations happening in the real robot experiments. 
\begin{enumerate}
	...
	\item to account for the vision software's detection errors, the raw data gathered from each experiment was de-bounced and passed through a low-pass filter to expose real collaboration events while eliminating observation error (see Figure \ref{fig:data clean}). 
\end{enumerate}

\begin{figure}[!htb]
\centering\includegraphics[width=\textwidth]{figures/expdat.png}
\centering\caption{A snippet of raw data of physical experiments is seen in the top figure, in blue. It shows number of agents with green leds on (i.e. collaborating) over time as detected by the overhead camera system. The figure below shows time adjusted data (solid green) with a low-pass added for debouncing (dashed red). The peaks of this fixed data were counted to count the number of collaborations over the course of a 30 minute experiment.}\label{fig:datfix}
\end{figure}



\begin{table}[!htb]
\centering\begin{tabular}{|*{6}{c|}}
\hline
  $\theta/\tau$  &       2 &       3 &       4 &       5 & 6\\
\hline
0.1 & 31.7776 & 26.1667 & 26.1667 & 24.5667 & 25.1333\\
\hline
1   & 20.7667 & 16.9667 & 11.0667 & 15.1    & 6.93333\\
\hline
10  & 0.76667 & 0.13333 & 1.23333 & 0       & 0\\
\hline
\end{tabular}
