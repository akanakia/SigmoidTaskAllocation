...
\subsection{Main Theorem and Proof}\label{subsec:thmproof}
...
Theorem~\ref{thrm:mainthrm} states that a communication-free agent-level threshold strategy is sufficient and necessary (from Lemma~\ref{lemma:thresholdBR}) to achieve a task-allocation resulting in system-level equilibrium for concurrent benefit tasks. This is a powerful result for the distributed multi-agent task-assignment problem but there are a few issues to consider before its practical application to all prior works.

First and foremost is the issue of applying this theory to continuous response-threshold functions such as sigmoid functions. Continuous response-threshold functions are an indispensable solution to the task-assignment problem in both natural and artificial systems, as is evident from the body of work referenced at the beginning of Section \ref{sec:rw}. The major benefit of using sigmoid vs. step functions is their flexibility, allowing swarm systems to quickly adapt to changes in task and environment parameters \cite{Bonabeau1997}. We provide a brief semi-formal argument to encompass them within our theorem.

Secondly, we wish to draw a clear distinction between system equilibrium policies versus optimum strategies. It is obvious that a fully distributed strategy for task-allocation with no communication between agents performs poorly for many optimality metrics when compared to a centralized approach where all information about the state of the system is known to the task-assignment controller. System optimality metrics could be allocation convergence speed or the rate of completion of assigned tasks. Somewhere between a centralized approach and a communication-free distributed one lies a strategy where some information (such as their noisy estimates, $x_i$) is shared between agents. We briefly discuss the work that has been done as well as the possibility of future research in this area.

\subsection{Why Sigmoid functions?}\label{subsec:sigfun}
Here, we argue that Theorem~\ref{thrm:mainthrm} does not contradict the continuous response-threshold model for MAS task-allocation but instead provides a serious argument for prevalence of such phenomena. 
...
\subsection{System Equilibrium vs. System Optimum}
...
When representing the task-allocation problem as a global game, an equilibrium is said to be achieved when all agents use a threshold strategy based on their incomplete knowledge of the task's magnitude, $\tau$. No agent can deviate from this strategy to receive a better payoff, defined by $u_i$---hence the term \emph{Best Response} strategy. The terms \emph{payoff} and \emph{utility} are often used interchangeably in game theory literature\ldots

This is definitely very interesting but it is hard to say anything mathematically. The system optimum is something that one should define. One definition would be the optimal task-allocation once a central agent knows all the information (sensor measurements, number of agents,…) in the system. Its performance would be definitely much better than none central allocation and this setting of global games where the information is shared globally\ldots

We also wish to briefly discuss the effects of information sharing between agents\cite{Touri2014}.
