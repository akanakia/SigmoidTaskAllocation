...
\def\td{\edit{\Upsilon}}   % response-threshold value
...
%The rest of this paper is organized as follows. The next section lays the foundation for the Bayesian game theory model and subsequent results of modeling response threshold based multi-agent task allocation as a global game. In section \ref{sec:globalgame} we also formally define a \emph{concurrent benefit} task which is central to our study. The \emph{best response} strategy for an agent using a response threshold based approach for task allocation is also defined here. Best response strategies are a common concept in game theory and provide the building blocks for many seminal results such as the Nash Equilibrium. We discuss their relevance to our model and define what they mean in a global games context. Section \ref{sec:discretethreshold} introduces the first major contribution of this paper---A Theorem that proves the best response to a response threshold strategy profile is also a response threshold strategy profile. The result follows that a response threshold strategy for communication free task allocation in multi-agent systems results in a Bayesian Nash Equilibrium. The following section generalizes Theorem 1 not only to discrete response thresholds but to continuous response threshold strategies as are often used to model natural task allocation systems \citep{Bonabeau1999}. The Discussion and Conclusion sections provide an analysis of these two theorems while grounding them in application domains that could benefit from these results. We finally also provide our future goals for this work.
...
We  note that the proposed task allocation mechanism is not optimal in terms of the allocations it can generate, but simply the best strategy for an individual agent given that other agents are using the same policy, which is the definition of a Nash equilibrium. This strategy is therefore only interesting if communication between agents is impossible or otherwise undesireable. In this case, the presented results provide an analysis framework, which makes such an approach viable in an engineering context. Yet, further work is needed to formally show what the lower performance bounds of such a policy are. 
...
\section{Conclusion}
\edit{We are studying a class of task allocation problems that require joint action of a certain average number of agents in order to be successful and in which agents do not communicate, but only have access to a common (noisy) stimulus signal. Due to the limited information available to individuals in such systems, probabilistic response threshold algorithms have emerged to be the first choice in both engineered and natural systems such as social insects, among others, that have limited computational and communication abilities. We show that this situation is akin to joint actions in human populations that are only successful when a critical mass is reached, specifically bank runs and revolutions, which are governed by very similar constraints. This insight allows us to leverage analysis from the field of game theory, which describes such a situation as a global game. Building up on existing proofs, we show that such a policy leads to a Bayesian Nash Equilibrium, that is, on average, no player has anything to gain by changing only their own strategy. Albeit this result does not imply that such a policy results in an optimal allocation, it provides --- for the first time --- a formal understanding for why a response threshold strategy is desirable.}  

\edit{Furthermore, we show how a noisy perception of a global, deterministic threshold signal can be interpreted as a deterministic observation of a sigmoidal threshold function, which are commonly observed or applied in natural or engineered swarms, respectively. As these observations can be explained by a noisy observation of a deterministic threshold, we believe this result to open an avenue for further study of social insect using methods and tools provided by the field of global games. From an engineering perspective, this finding shows that the balance between exploration and exploitation can simply be tuned by adding or removing noise to a robot's perception apparatus.}
