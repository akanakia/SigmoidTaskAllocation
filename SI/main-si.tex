...
\edit{The existence of a BNE in a global game is contingent on individuals not communicating with each other. This is immediately obvious as the availability of information to some agents, but not to others, might allow them to improve some global metric by changing only its own policy. As both engineered and natural systems could easily achieve better results by communicating (see related work on optimal task allocation), we interpret probabilistic response threshold task allocation mechanisms to be a suitable baseline strategy in engineered systems. In natural systems instead, there existence could serve as a clue that individuals either do not communciate directly for one or the other reason.}
