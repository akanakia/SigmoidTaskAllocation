...
The firefighting and bank run, where agents act collaboratively or adversarial, respectively, may at first seem orthogonal. Both scenarios, however, share the notion that to be successful, an agent not only need to assess the magnitude of the task itself, but also the likelihood of the other agents to act. 
NC: NEED TO CONTINUE HERE
We hypothesize that a collective group's observed behavior can be effectively emulated by assuming each agent makes their individual decision on whether or not to perform a task based on some internal threshold value which is compared to their noisy estimates of the collective task's magnitude-$\tau$. 
